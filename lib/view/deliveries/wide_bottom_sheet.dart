...
class WideBottomSheet extends StatefulWidget {
  final int ballType;
  final ScoringDetailResponseModel? scoringData;
  final VoidCallback refresh;
  const WideBottomSheet(this.ballType, this.scoringData, this.refresh, {super.key});

  @override
  State<WideBottomSheet> createState() => _WideBottomSheetState();
}

class _WideBottomSheetState extends State<WideBottomSheet> {
                      ...
                      onTap:()async{
                        ...
                        if(isWideSelected!=null) {
                          ...
                          scoreUpdateRequestModel.runsScored =
                              1 + (isWideSelected ?? 0);
                          scoreUpdateRequestModel.extras = 1 + (isWideSelected ?? 0);
                          scoreUpdateRequestModel.extrasSlug = isWideSelected;
                          ...
                          WidgetsBinding.instance.addPostFrameCallback((_) {
                            Dialogs.snackBar("Please select one option", context, isError: true);
                          });
                        }
                      },
